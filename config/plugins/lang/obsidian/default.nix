  ...
  in
    mkIf enable {
      extraConfigLuaPost = ''
          ...
          local group = vim.api.nvim_create_augroup("FiletypeKeymaps", { clear = true })
          vim.api.nvim_create_autocmd("FileType", {
            ...
            callback = function()
              ...
              map("n", "<leader>io", function()
                ...
                vim.fn.delete(temp_dir, "rf")
              end, { desc = "Extract text from image", remap = false, buffer = true });
              ...
              end, { desc = "Rename image file", remap = false, buffer = true });
              ...
              end, { desc = "Delete image file under cursor", remap = false, buffer = true });
              map("n", "<leader>oT", function()
                vim.cmd("norm gg")
                vim.cmd.Obsidian("template", "note")
                vim.cmd(":%s/^#\\s\\d\\+-\\(.\\+\\)/# \\1/g")
              end, { desc = "Insert Obsidian Template", remap = false, buffer = true });
              ...
              end, { desc = "Paste image into obsidian note", remap = false, buffer = true });
              map("n", "<leader>or", ":Obsidian rename<cr>", { desc = "Rename current obsidian note", remap = false, buffer = true });
              map("n", "<leader>ob", ":Obsidian backlinks<cr>", { desc = "Show backlinks for current obsidian note", remap = false, buffer = true });
              ...
              end, { desc = "Add surrounding to URL", remap = false, buffer = true });
              map("v", "sc", function()
              ...
              end, { desc = "Make selection inline code", remap = false, buffer = true });
              map("v", "si", function()
                -- Use nvim_replace_termcodes to handle special characters like backticks
                local keys = vim.api.nvim_replace_termcodes("sa*", true, false, true)
                vim.api.nvim_feedkeys(keys, "x", false)
              end, { desc = "Make selection italic", remap = false, buffer = true });
              map("v", "sb", function()
                -- Use nvim_replace_termcodes to handle special characters like backticks
                local keys = vim.api.nvim_replace_termcodes("2sa*", true, false, true)
                vim.api.nvim_feedkeys(keys, "x", false)
              end, { desc = "Make selection bold", remap = false, buffer = true });
              map("n", "<leader>mB", function() vim.cmd("normal Vsb") end, { desc = "Make line under cursor bold", remap = false, buffer = true });
              ...
              end, { desc = "Toggle bold marker on current word", remap = false, buffer = true });
              map("n", "<leader>mI", function() vim.cmd("normal Vsi") end, { desc = "Make line under cursor italic", remap = false, buffer = true });
              map("n", "<leader>mi", function()
                vim.cmd("normal saiw*")
              end, { desc = "Make word under cursor italic", remap = false, buffer = true });
            end,
        })
      '';
      keymaps = [
